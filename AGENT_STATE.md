...
Last Updated: 2026-07-16 15:57 CST
...
- Trading dashboard lives in `dashboard/`: `server.py` serves a zero-build HTML/CSS/JS interface plus market-data and journal APIs; `config.json` holds holdings, candidate peer cohorts, and overseas peer mappings. It has four modules: today's permission gate, A-share holding-vs-peer arena with intraday peer charts and embedded prior-session overseas peers, post-close sector/concept flow, and a daily Markdown journal browser/editor. Start with `python3 dashboard/server.py`, then open `http://127.0.0.1:8765`.
