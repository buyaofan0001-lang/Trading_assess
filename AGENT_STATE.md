...
Last Updated: 2026-07-16 14:09 CST
...
- Initial trading dashboard lives in `dashboard/`: `server.py` serves a zero-build HTML/CSS/JS interface and `/api/dashboard`; `config.json` holds holdings, candidate peer cohorts, US mappings, and the recovery checklist. It has five modules: today's permission gate, A-share holding-vs-peer arena, prior-session US mapping, post-close sector/concept flow, and a locally persisted life-recovery checklist. Start with `python3 dashboard/server.py`, then open `http://127.0.0.1:8765`.
