...
Last Updated: 2026-07-16 14:54 CST
...
- Trading dashboard lives in `dashboard/`: `server.py` serves a zero-build HTML/CSS/JS interface plus `/api/dashboard`, `/api/peer-groups`, and `/api/intraday`; `config.json` holds holdings, candidate peer cohorts, US mappings, and the recovery checklist. It has five modules: today's permission gate, A-share holding-vs-peer arena with intraday peer charts, prior-session US mapping, post-close sector/concept flow, and a locally persisted life-recovery checklist. Start with `python3 dashboard/server.py`, then open `http://127.0.0.1:8765`.
