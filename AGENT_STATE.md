...
Last Updated: 2026-07-16 15:22 CST
...
- Trading dashboard lives in `dashboard/`: `server.py` serves a zero-build HTML/CSS/JS interface plus market-data and journal APIs; `config.json` holds holdings, candidate peer cohorts, US mappings, and the recovery checklist. It has six modules: today's permission gate, A-share holding-vs-peer arena with intraday peer charts, prior-session US mapping, post-close sector/concept flow, a daily Markdown journal browser/editor, and a locally persisted life-recovery checklist. Start with `python3 dashboard/server.py`, then open `http://127.0.0.1:8765`.
